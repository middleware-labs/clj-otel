...
https://github.com/middleware-dev/clj-otel[`clj-otel`] extends the reach of OpenTelemetry to *Clojure libraries and applications* by providing:
...
https://github.com/middleware-dev/clj-otel[`clj-otel`] is an umbrella project for several Clojure modules `clj-otel-*`.
...
* https://www.splunk.com/en_us/products/observability.html[Splunk Observability Cloud]
...
See this https://opentelemetry.io/docs/reference/specification/metrics/supplementary-guidelines/#instrument-selection[OpenTelemetry guide] for selecting the appropriate instrument to use.
...
It is a default for many functions in this project `clj-otel` and methods of the underlying Java library `opentelemetry-java`.
The current context is safe to use when manually instrumenting synchronous code.

WARNING: Do not use the current context when manually instrumenting asynchronous code.
